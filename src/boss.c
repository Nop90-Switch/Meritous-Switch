...
void Curse()
{
	...
	rooms[0].room_type = 2;
	Paint(rooms[0].x+1, rooms[0].y+1, rooms[0].w-2, rooms[0].h-2, "/switch/meritous/dat/d/fbossroom.loc");
...
void DrawBossHP(int bar_length)
{
	...
	if (boss_hp_icon == NULL) {
		boss_hp_icon = IMG_Load("/switch/meritous/dat/i/boss_icon.png");
	}
...
void TryHurtBoss(int x, int y, int range, int power)
{
	...
	if (dist(x, y, boss_x, boss_y) <= range) {
		...
		if (atk_power > boss_breakpoint) {
			...
			SND_Pos("/switch/meritous/dat/a/enemyhit.wav", 128, dist(x, y, boss_x, boss_y) / 4);
			boss_flash = 40;
			if (boss_hp <= 0) {
				boss_new_life = 1;
				boss_hp = 0;
...
void DrawBoss()
{
	...
	switch (current_boss) {
		case 0: {
			...
			SDL_Rect drawpos;
			if (boss_spr == NULL) {
				boss_spr = IMG_Load("/switch/meritous/dat/i/boss1.png");
		...
		case 1: {
			...
			SDL_Rect drawpos;
			if (boss_spr == NULL) {
				boss_spr = IMG_Load("/switch/meritous/dat/i/boss2.png");
				SDL_SetColorKey(boss_spr, SDL_SRCCOLORKEY | SDL_RLEACCEL, 255);
			}
		...
		case 2: {
			...
			if (boss_spr == NULL) {
				boss_spr = IMG_Load("/switch/meritous/dat/i/boss3.png");
		...
		case 3: {
			...
			if (boss_spr == NULL) {
				boss_spr = IMG_Load("/switch/meritous/dat/i/boss4.png");
				SDL_SetColorKey(boss_spr, SDL_SRCCOLORKEY | SDL_RLEACCEL, 0);
			}
			...
			if (bdef > 0) {
				...
				if (t_timer < 220) {
					...
					if (boss_fight_mode < 23) {
						...
						if (endpics[0] == NULL) {
							if (training) {
								endpics[0] = IMG_Load("/switch/meritous/dat/i/wuss_ending.png");
							} 
						}
