...
void InitHelp()
{
	...
	fp = fopen("/switch/meritous/dat/d/helpfile.txt", "r");
	while (!feof(fp)) {
		fgets(linebuf, 79, fp);
...
//			if (ev.key.keysym.sym == SDLK_h) {
			if ((ev.key.keysym.sym == SDLK_x) || (ev.key.keysym.sym == SDLK_MINUS)) {
				return 0;
			}
//			if ((ev.key.keysym.sym == SDLK_SPACE) || (ev.key.keysym.sym == SDLK_RETURN))
			if ((ev.key.keysym.sym == SDLK_a) || (ev.key.keysym.sym == SDLK_b) )
				my_link = 1;
			}
