...
void KillEnemy(struct enemy *t)
{
	...
	if ((ct - lastkill) > 100) {
		SND_Pos("/switch/meritous/dat/a/enemyhit.wav", 128, PlayerDist(t->x, t->y));
		lastkill = ct;
	}
...
void EnemySound(int t, int dist)
{
	...
	switch (t) {
		case 0:
			SND_Pos("/switch/meritous/dat/a/mons0shot.wav", 48, dist);
			last_delay = 200;
		break;
		case 1:
			SND_Pos("/switch/meritous/dat/a/mons1shot.wav", 112, dist);
			last_delay = 500;
		break;
		case 2:
			SND_Pos("/switch/meritous/dat/a/mons2shot.wav", 110, dist);
			last_delay = 1000;
		break;
		case 3:
			SND_Pos("/switch/meritous/dat/a/mons3shot.wav", 110, dist);
			last_delay = 500;
		break;
		case 4:
			SND_Pos("/switch/meritous/dat/a/mons4shot.wav", 110, dist);
			last_delay = 900;
		break;
		case 5:
			SND_Pos("/switch/meritous/dat/a/mons5shot.wav", 80, dist);
			last_delay = 60;
		break;
		case 6:
			SND_Pos("/switch/meritous/dat/a/mons6shot.wav", 110, dist);
			last_delay = 1000;
		break;
		case 7:
			SND_Pos("/switch/meritous/dat/a/mons7shot.wav", 110, dist);
			last_delay = 600;
		break;
		case 8:
			SND_Pos("/switch/meritous/dat/a/mons8shot.wav", 110, dist);
			last_delay = 700;
		break;
		case 9:
			SND_Pos("/switch/meritous/dat/a/mons9shot.wav", 110, dist);
			last_delay = 242;
		break;
		case 10:
			SND_Pos("/switch/meritous/dat/a/mons10shot.wav", 110, dist);
			last_delay = 250;
		break;
						...
						if (player_dying == 0) {
							if (PlayerDist(fx, fy) < 30) {
								...
								if (PlayerDist(fx, fy) < 4 - (2 * artifacts[5])) {
									player_dying = 1;
									SND_Pos("/switch/meritous/dat/a/playerhurt.wav", 128, 0);
									e->dying = 1;
									break;
								}
							}
						...
						if (shield_hp > 0) {
							...
							if (e->img == 4) {
								...
								if ((c_shield_hit_sound - 150) > last_shield_hit_sound) {
									SND_Pos("/switch/meritous/dat/a/shieldhit.wav", 50, 0);
...
void DrawEnemy(struct enemy *e, SDL_Surface *scr)
{
	...
	if (e->teleport_v < 24) {
		if (teleflash == NULL) {
			teleflash = IMG_Load("/switch/meritous/dat/i/teleflash.png");
...
void DrawBullet(struct bullet *b)
{
	...
	if (b->img == 1) {
		if (d_star_big == NULL) {
			d_star_big = IMG_Load("/switch/meritous/dat/i/star1.png");
	...
	if (b->img == 3) {
		if (d_star_small == NULL) {
			d_star_small = IMG_Load("/switch/meritous/dat/i/star2.png");
	...
	if (b->img == 6) {
		if (d_star_ls == NULL) {
			d_star_ls = IMG_Load("/switch/meritous/dat/i/star3.png");
...
void DrawGem(struct diamond *g)
{
	...
	if (d_sprite == NULL) {
		d_sprite = IMG_Load("/switch/meritous/dat/i/gem.png");
				...
				if (PlayerDist(g->x, g->y) < 20) {
					...
					if (g->value == 31337) {
						if (player_hp < (3 + (player_shield == 30)*3)) {
							SND_Pos("/switch/meritous/dat/a/crystal.wav", 64, 0);
							player_hp++;
						} else {
							if (!training) {
								SND_Pos("/switch/meritous/dat/a/tone.wav", 64, 0);
								...
								if (player_lives_part >= 88) {
									...
									player_lives += 1;
									SND_Pos("/switch/meritous/dat/a/crystal2.wav", 100, 0);
								}
							}
						}
