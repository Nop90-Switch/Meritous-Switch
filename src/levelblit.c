...
void VideoUpdate()
{
	...
	if (WriteBitmaps) {
		if ((bmp >= WB_StartRange)&&(bmp < WB_EndRange)) {
			sprintf(bmp_name, "/switch/meritous/bmp%d.bmp", bmp);
...
// switch
	...
	asceai = IMG_Load("/switch/meritous/dat/i/asceai.png");
	wm_icon = IMG_Load("/switch/meritous/dat/i/icon.png");
...
//	screen = SDL_SetVideoMode(SCREEN_W, SCREEN_H, 8, SDL_SWSURFACE | (SDL_FULLSCREEN * fullscreen));
	...
	wm_mask_file = fopen("/switch/meritous/dat/d/icon_bitmask.dat", "rb");
	...
	while (executable_running) {
		...
		title = IMG_Load("/switch/meritous/dat/i/title.png");
		title_pr = IMG_Load("/switch/meritous/dat/i/title.png");
		...
		if (executable_running == 1) {
			...
			if ((option == 0) && can_continue) {
				DungeonPlay("/switch/meritous/SaveFile.sav");
			} else {
				if (option == (0 + can_continue)) {
					training = 0;
					DungeonPlay("");
...
void DrawMeter(int x, int y, int n)
{
	...
	if (meter == NULL) {
		meter = IMG_Load("/switch/meritous/dat/i/meter.png");
			...
			if (player_room == agate_knife_loc) {
				{
					...
					SDL_Rect draw_to;
					if (agate_knife == NULL) {
						agate_knife = IMG_Load("/switch/meritous/dat/i/agate.png");
					...
					if (dist(player_x, player_y, xpos, ypos) < 20) {
						...
						SND_Pos("/switch/meritous/dat/a/crystal2.wav", 128, 0);
...
void DrawLevel(int off_x, int off_y, int hide_not_visited, int fog_of_war)
{
	...
	if (tiles == NULL) {
		tiles = IMG_Load("/switch/meritous/dat/i/tileset.png");
		fog = IMG_Load("/switch/meritous/dat/i/tileset.png");
...
void DrawPlayer(int x, int y, int pl_dir, int pl_frm)
{
	...
	if (playersprite == NULL) {
		playersprite = IMG_Load("/switch/meritous/dat/i/player.png");
...
void text_init()
{
	FILE *font_data_file;
	int chr, x, y;
	font_data_file = fopen("/switch/meritous/dat/d/font.dat", "rb");
		...
		while (!given_treasure) {
			...
			switch (treasure) {
				case 0:
					...
					given_treasure = 1;
					SND_Pos("/switch/meritous/dat/a/tone.wav", 128, 0);
					break;
				case 1:
					if (player_shield < 25) {
						specialmessage = 10;
	...
	switch (tile) {
		...
		case 28:
			...
			if (player_gems >= UpgradePrice(0)) {
				...
				player_shield += 1;
				SND_Pos("/switch/meritous/dat/a/crystal.wav", 128, 0);
			}
			break;
		case 29:
		...
		case 32:
			CrystalSummon();
			SND_Pos("/switch/meritous/dat/a/crystal.wav", 80, 0);
			break;
		default:
			break;
	}
...
void DrawArtifacts()
{
	...
	if (artifact_spr == NULL) {
		artifact_spr = IMG_Load("/switch/meritous/dat/i/artifacts.png");
